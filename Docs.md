...
Mods must met some requirements, eg. contains all the config files. (See [Mod Structure](#Mod-Structure))
...
-*behavior* : represents lua script. (See [Behaviors](#Behaviors)) Example:  
 ```json
{
    "asset_type" : "behavior",
    "path" : "$/controller.lua"
...
```
-*texture* : represents texture. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "texture",
...
```
-*sprite_sheet* : represents sprite sheet. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "flipbook",
...
```
-*flipbook* : represents sprite sheet, and provide information about animations. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "flipbook",
    "path" : "$/player_sprite_sheet.png",
...
```
-*shader* : represents GLSL shader. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "shader",
...
```
-*tileset* : represents tileset and provide infromation about which tiles (Numbering from top-left corner, starting at 1) should have collision. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "tileset",
    "path" : "$/tileset.png",
...
```
-*tilemap* : represents tilemap generated by tiled program. (See [Renderer](#Renderer)) Example:
```json
{
    "asset_type" : "tilemap",
    "path" : "$/tilemap.tmj"
...
```
-*scene* : represents lua script that create, and manage an scene. (See [Scenes](#Scenes)) Example:
```json
{
    "asset_type" : "scene",
    "path" : "$/scene.lua"
...
```
-*sound* : represents mp3 sound. (See [Audio System](#Audio-System)) Example:
```json
{
    "asset_type" : "sound",
    "path" : "$/orchestra.mp3"
...
```
-*mesh* : represents an mesh (geometry). (See [Renderer](#Renderer)) Example: 
```json
{
    "asset_type" : "mesh",
...
```
-*rendering_config* : contains renderer configuration. (See [Renderer](#Renderer)) Example:
```json
{
...
```
-*input_config* : contains input subsystem configuraion. (See [Input System](#Input-System)) Example:
```json
{
...
``` 
-*collision_config* : contains collision presets configuration (See [Collision System](#Collision-System)) Example:
```json
{
...
```lua 
entity_ref      _e_create()                                             --creates entity inside current scene and returns reference to it.
...
table           _e_call(entity_ref e, string event_name, table args)    --calls event of given name on all behaviors components inside the entity. Returns table of event calls return values.
